...
CPair ClosestInParts(node Points[],int s,int r)
{
	if (r-s<3)
	{
		//printf("s and r is %d and %d\n",s,r);
		return MininThree(Points,s,r);
	}
	else
	{
		//printf("here is else\n");
		int mid=(s+r)/2;
		node Median=Points[mid];
		//printf("Median is %d %d\n",Median.x,Median.y );
		CPair minL=ClosestInParts(Points,s,mid-1);
		CPair minR=ClosestInParts(Points,mid+1,r);
		...
		{
			//printf("here i am\n");
			if (abs(Points[i].x-Median.x)<minD.minV)
			{
				Strip[size]=Points[i];
...
{
	qsort(Points,r-s+1,sizeof(node),cmp);
	/*int i;
	for (i=s;i<=r; ++i)
	{
	...
	printf("\n");*/
	return ClosestInParts(Points,s,r);
}
